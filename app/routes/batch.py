...
@router.post("/api/VerifySignatureBatch", response_model=BatchCompareResponse)
async def verify_signature_batch(
    ...
    reference_images: list[UploadFile] = [
        img
        for img in [ref_1, ref_2, ref_3, ref_4, ref_5, ref_6, ref_7, ref_8, ref_9, ref_10]
        if img is not None
    ]
    ...
    async def _single_comparison(ref_bytes: bytes, ref_name: str, index: int) -> IndividualResult:
        ...
        try:
            resp = await compare_signatures(
                image1_bytes=ref_bytes,
