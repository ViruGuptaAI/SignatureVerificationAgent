...
def batchSummaryPrompt(majority_matched: bool, match_count: int, total_count: int,
    ...
    return (
        f"## Role\n"
        f"You are a summarization agent. Your sole job is to read the individual "
        f"outputs produced by {total_count} signature-verification agents and "
        f"synthesise them into one cohesive, professional paragraph.\n\n"

        f"## Context\n"
        f"A test signature was compared against {total_count} reference samples.\n"
        f"Aggregate verdict: {verdict_label} "
        ...
        f"1. Produce exactly ONE paragraph of 2-3 sentences in plain prose.\n"
        f"2. Open with the overall verdict, then cite the dominant visual evidence "
        f"(stroke dynamics, pressure patterns, letterform construction, etc.).\n"
        f"3. If the agents disagree, briefly note the conflict and which view prevailed.\n"
        f"4. Use formal, authoritative forensic language throughout.\n"
        f"5. Do NOT repeat numeric statistics (match ratio, confidence scores) — "
        f"these are already displayed to the user.\n"
        f"6. Do NOT use bullet points, markdown formatting, hedging, or filler phrases."
    )
